...
async function onSuccess(result) {
  ...
  // console.log(route.shape[0]);
  // console.log(route.shape[0].split(","));
  // console.log(parseFloat(route.shape[0].split(",")[0]));
  // console.log(parseFloat(route.shape[0].split(",")[1]));

  var points = [];

  for(var i =0; i<route.shape.length;i++){
    var parsed = route.shape[i].split(",");
    var lat = parseFloat(parsed[0]);
    var long = parseFloat(parsed[1]);
    points[i] = {
          "lat": lat,
          "lng": long
        };
